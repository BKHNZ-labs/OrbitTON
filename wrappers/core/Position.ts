import {
  ...
  import { BurnPositionMessage, BurnPositionParams, CollectMessage, CollectParams, storeBurnPositionMessage, storeCollectMessage } from '../../tlb/position/messages';
import { loadPositionStorage } from '../../tlb/position';
    ...
    export class Position implements Contract {
      ...
      async getStorage(provider: ContractProvider) {
        ...
        if (storage.state.type === 'active') {
          return loadPositionStorage(Cell.fromBoc(Buffer.from(storage.state.data ?? Buffer.from([])))[0].beginParse());
        }   
      ...
      async sendCollect(provider: ContractProvider, via: Sender, value: bigint, recipient: Address, amount0Requested: bigint, amount1Requested: bigint) {
