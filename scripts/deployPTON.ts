...
export async function run(provider: NetworkProvider) {
  const router = provider.open(
    RouterWrapper.RouterTest.createFromAddress(Address.parse('EQAQW9X_hfjQhG3F7Oo_fwP3Ty9lpVnYTydGRcezhaI7nED_')),
  );

  const pTonMaster = provider.open(
    PTonMinterWrapper.PTonMinterV2.createPtonMinterFromAddress(
      Address.parse('kQBnGWMCf3-FZZq1W4IWcWiGAc3PHuZ0_H-7sad2oY00o3ZY'),
    ),
  );
  ...
  const routerPTONWallet = provider.open(PTonMinterWrapper.PTonMinterV2.createFromAddress(routerPTONAddress));
